# Submission Draft: octintri

**To:** Erich Friedman (via packing website)
**Subject:** New packing records for octintri (8 in 3)
**Problem Page:** https://erich-friedman.github.io/packing/octintri/index.html

---

Dear Dr. Friedman,

This is Luke Kaiser. We have found improved packing configurations for **octintri** (8 in 3):

### Methodology
We discovered these packings using a continuous global optimization solver combining randomized geometric basin-hopping with Separating Axis Theorem (SAT) collision constraints and SLSQP local gradient refinement. All solutions have been validated with 0 pairwise overlaps and strict container boundary containment.

### New Packings
| N | Our s (5 dec) | Our Exact s | Friedman Best s | Improvement | Attached Image |
| :---: | :---: | :---: | :---: | :---: | :---: |
| 6 | 9.31578+ | 9.315779776455 | 9.31580+ | -0.000020 | `6.png` |

### Image Attachments
The replacement image files are attached directly with exact matching colors (#feffe9 on #ffffff), orientation, no borders or text, and target website pixel dimensions (250x216 px):
- `6.png`

Coordinate tables and certificates for each solution are included in the repository records.

Sincerely,
Luke Kaiser
